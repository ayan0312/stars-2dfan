...
import { IDownloadedInformation, downloadImage, getHTMLString } from '../../core'
import { getURLRouter, merge, filter2DFanImageURL, verifyURL, delay, format, getMd5Name } from '../../utils'
...
export class Subject2DFan {
    ...
    public static async getInformation(URL: string, add: object = {}): Promise<IGameInformation> {
        try {
            ...
            const date: string = format('y-M-d,H:m:s', timestamp)
            group = merge(group, {
                date,
    ...
    public static async mergeDownloadedInformation(
        ...
        const name: string = getMd5Name(group.name, group._id.toHexString())
